...
function DeviceStatusBar({ tone = "dark" }) {
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: STATUS_H, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 22px", pointerEvents: "none" }}>
      <span style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 15, color: col }}>9:41</span>
      <div style={{ position: "absolute", top: 9, left: "50%", transform: "translateX(-50%)", width: 108, height: 26, background: "#000", borderRadius: 16 }} />
...
/* Brand-accurate logo tiles (SVG recreations — swap for licensed official
   assets in production). Matches the reference screenshot's tile style. */
function BrandLogo({ mid, size = 44 }) {
  const r = size * 0.24;
  const tile = (bg, children) => (
    <svg width={size} height={size} viewBox="0 0 44 44" style={{ flex: "0 0 auto", borderRadius: r, boxShadow: "0 2px 6px rgba(0,0,0,.10)" }}>
  ...
  switch (mid) {
    case "amazon":
      return tile("#fff", <>
        <text x="22" y="24" textAnchor="middle" fontFamily="Arial, sans-serif" fontWeight="700" fontSize="12.5" fill="#131921">amazon</text>
        <path d="M11 28c6.5 4.6 16.2 4.4 22-.4" stroke="#FF9900" strokeWidth="2.4" fill="none" strokeLinecap="round" />
        <path d="M33 27.6l1.8-2.4-3 .5" fill="#FF9900" />
      </>);
    case "flipkart":
      return tile("#2874F0", <>
        <path d="M14 12h13l-1.2 4.6H21l-.7 3h4.6l-1.1 4.4h-4.6L17.6 32H13l4-20z" fill="#FFE11B" transform="translate(1,0)" />
      </>);
    case "meesho":
      return tile("#570D48", <>
        <text x="22" y="29" textAnchor="middle" fontFamily="Arial, sans-serif" fontWeight="800" fontSize="22" fill="#FDEFF4">m</text>
      </>);
    case "blinkit":
      return tile("#F8CB46", <>
        <text x="22" y="26.5" textAnchor="middle" fontFamily="Arial, sans-serif" fontWeight="800" fontSize="10.5" fill="#1C1C1C">blink<tspan fill="#0C831F">it</tspan></text>
      </>);
    case "zepto":
      return tile("#4F1D9E", <>
        <text x="22" y="30" textAnchor="middle" fontFamily="Arial, sans-serif" fontWeight="800" fontSize="24" fill="#fff">Z</text>
      </>);
    case "instamart":
      return tile("#FC8019", <>
        <text x="22" y="29" textAnchor="middle" fontFamily="Arial, sans-serif" fontWeight="800" fontSize="21" fill="#fff">S</text>
      </>);
    default:
      return tile("#eee", null);
  }
}
...
const MktTag = ({ mid }) => {
  ...
  return <span style={{ fontFamily: FONT_BODY, fontWeight: 800, fontSize: 10, letterSpacing: ".08em", color: C.sub }}>{m.emoji} {m.name.toUpperCase()}</span>;
};
...
function Onboarding({ go }) {
  ...
  return (
    <Screen bg={o.bg} noPad>
      <div style={{ position: "relative", flex: "1 1 auto", overflow: "hidden", transition: "background .4s" }}>
        <button onClick={() => go("authlanding")} style={{ position: "absolute", top: 50, right: 18, zIndex: 5, background: "rgba(255,255,255,.85)", border: "none", borderRadius: 20, padding: "7px 14px", fontFamily: FONT_BODY, fontWeight: 700, fontSize: 12, color: C.ink2, cursor: "pointer" }}>Skip</button>
        {ONBOARD_ART[o.art]
          ? <img src={ONBOARD_ART[o.art]} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", animation: "fayr-pop .5s ease both" }} />
          : <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", fontSize: 90 }}>{["🛍️", "⭐", "💸"][i]}</div>}
      </div>
      <div style={{ flex: "0 0 auto", background: C.cream, borderTopLeftRadius: "50% 44px", borderTopRightRadius: "50% 44px", marginTop: -46, padding: "38px 30px 24px", position: "relative", zIndex: 2, minHeight: 265, display: "flex", flexDirection: "column" }}>
        <div key={i} style={{ animation: "fayr-rise .45s ease both" }}>
...
function TruecallerSheet({ go, setName, setPhone, setTcName }) {
  return (
    <Screen>
      ...
      <div style={{ position: "relative", background: "#fff", borderTopLeftRadius: 26, borderTopRightRadius: 26, padding: "24px 24px 26px", animation: "fayr-rise .35s ease both", textAlign: "center" }}>
  ...
  return (
    <Screen>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 30, textAlign: "center", position: "relative" }}>
        ...
        <div style={{ fontSize: 58, animation: "fayr-pop .5s ease both" }}>👋</div>
  ...
  return (
    <div style={{ flex: "0 0 auto", background: "#fff", borderTop: `1px solid ${C.line}`, padding: "8px 10px 12px", display: "flex", boxShadow: "0 -6px 18px rgba(0,0,0,.05)" }}>
      {items.map(([k, icon, label]) => {
        const active = tab === k;
        return (
          <button key={k} onClick={() => go(k)} style={{ background: active ? "#FDF9DF" : "none", borderRadius: 12, border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, flex: 1, padding: "5px 0" }}>
            <span style={{ fontSize: 19, filter: active ? "none" : "grayscale(1)", opacity: active ? 1 : 0.55 }}>{icon}</span>
...
function CampaignCard({ c, enrolled, onClick, hero, delay = 0 }) {
  ...
  const e = enrolled[c.id] || {};
  return (
    <div onClick={onClick} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 6px 16px rgba(20,20,20,.06)", cursor: "pointer", width: hero ? "100%" : 220, flex: "0 0 auto", animation: "fayr-rise .5s ease both", animationDelay: delay + "ms", opacity: k === "missed" ? 0.75 : 1 }}>
      <div style={{ position: "relative" }}>
    ...
    <Screen noPad bg="#FBFBEF">
      ...
      <div style={{ background: `linear-gradient(180deg, ${C.headYellow} 0%, ${C.headYellow2} 78%, #FBFBEF 100%)`, padding: "0 18px 6px", flex: "0 0 auto", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ height: 96, display: "grid", placeItems: "center", position: "relative" }}>
          <div key={bi} style={{ textAlign: "center", animation: "fayr-pop .5s ease both" }}>
      ...
      {netState === "error" ? (
        ...
        <div className="fayr-scroll" style={{ flex: 1, overflowY: "auto", padding: "10px 0 170px" }}>
          ...
          {/* auto-advancing hero carousel with pop-out */}
          <HeroCarousel campaigns={CAMPAIGNS.filter((c) => ["c6", "c7", "c11", "c10", "c9"].includes(c.id))} onOpen={openCampaign} />

          {/* themed horizontal rails */}
          {RAILS.map((r) => {
            const list = CAMPAIGNS.filter((c) => c.rail === r.key);
            if (!list.length) return null;
            return (
              <div key={r.key} style={{ marginTop: 22 }}>
                <h2 style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 16.5, color: C.ink, margin: "0 0 12px", padding: "0 16px" }}>{r.title}</h2>
                <div className="fayr-scroll" style={{ display: "flex", gap: 12, overflowX: "auto", padding: "2px 16px 4px", scrollSnapType: "x mandatory" }}>
                  {list.map((c, i) => <MiniCard key={c.id} c={c} onOpen={() => openCampaign(c)} delay={i * 50} />)}
                </div>
              </div>
            );
          })}
      ...
      {/* floating reminder card — sits just above the bottom nav (per reference) */}
      {!reminderDismissed && (
        <div style={{ position: "absolute", left: 0, right: 0, bottom: 74, padding: "0 12px", zIndex: 20, pointerEvents: "none" }}>
...
function MiniCard({ c, onOpen, delay = 0 }) {
  return (
    <div onClick={onOpen} style={{ width: 168, flex: "0 0 auto", scrollSnapAlign: "start", background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 5px 14px rgba(20,20,20,.07)", cursor: "pointer", animation: "fayr-rise .5s ease both", animationDelay: delay + "ms" }}>
...
function ClaimCard({ c, enrolled, claimed, onOpen, continueStep, delay = 0 }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 18, marginBottom: 14, boxShadow: "0 6px 16px rgba(20,20,20,.07)", animation: "fayr-rise .5s ease both", animationDelay: delay + "ms", overflow: "hidden", position: "relative" }}>
      {c.ribbon && (
      ...
      <div style={{ display: "flex", gap: 12, padding: "14px 14px 10px" }}>
        ...
        <div style={{ position: "relative", width: 108, height: 122, flex: "0 0 auto" }}>
          ...
          <div style={{ position: "absolute", left: "50%", bottom: -1, transform: "translateX(-50%)", background: "#fff", borderRadius: "8px 8px 0 0", padding: "3px 8px 2px", boxShadow: "0 -2px 6px rgba(0,0,0,.06)" }}>
            <BrandLogo mid={c.marketplace} size={22} />
          </div>
        </div>
...
function HowWorksVideo({ go, c }) {
  return (
    <Screen noPad bg="#FBFBEF">
      ...
      <div style={{ position: "relative", margin: "0 14px 26px", background: "linear-gradient(180deg,#FDF6C9,#FFFDF2)", borderRadius: 22, padding: "20px 18px 22px", animation: "fayr-rise .35s ease both", boxShadow: "0 -10px 40px rgba(0,0,0,.25)" }}>
...
function Detail({ go, c, enrolled, claimed, claim, buyNow, linked }) {
  ...
  const steps = [
    ...
    ["📦", `Buy Product on ${m.name}`, "as you always do"],
    ["🧾", "Submit order details", "Order ID + invoice, takes under a minute."],
    ["⭐", "Submit Your fayr Feedback", "An honest review once you've tried it."],
    ["💸", "Get Refund", "After the return window ends. Straight to your wallet."],
  ];
  return (
    <Screen noPad bg="#FBFBEF">
      {/* tinted hero with product image */}
      <div style={{ position: "relative", background: `linear-gradient(180deg, ${c.heroBg}, #FBFBEF)`, flex: "0 0 auto", paddingBottom: 6 }}>
        <StatusSpacer />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "2px 16px" }}>
          <button onClick={() => go("home")} style={{ width: 38, height: 38, borderRadius: "50%", border: "none", background: "#fff", fontSize: 15, cursor: "pointer", boxShadow: "0 4px 10px rgba(0,0,0,.1)" }}>⌄</button>
          <div style={{ background: "#fff", borderRadius: 16, padding: "4px 10px", boxShadow: "0 2px 8px rgba(0,0,0,.08)" }}><BrandLogo mid={c.marketplace} size={24} /></div>
        </div>
        <div style={{ display: "grid", placeItems: "center", padding: "6px 0 12px" }}>
          <ProductArt theme={c.theme} cid={c.id} radius={20} style={{ width: 168, height: 178, animation: "fayr-pop .5s ease both", background: "transparent" }} />
        </div>
        <div style={{ display: "flex", justifyContent: "center", paddingBottom: 8 }}><ProgressDots n={4} i={0} /></div>
      </div>

      <div className="fayr-scroll" style={{ flex: 1, overflowY: "auto", padding: "0 16px 104px" }}>
        {/* title card */}
        <div style={{ background: "#fff", borderRadius: 16, padding: 14, boxShadow: "0 4px 12px rgba(20,20,20,.06)", marginTop: -2 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: C.refundBg, borderRadius: 6, padding: "3px 9px", fontFamily: FONT_DISPLAY, fontWeight: 700, fontStyle: "italic", fontSize: 13, color: C.refundInk }}>Get {c.pct}% Refund 💵</span>
          <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 17, color: C.ink, marginTop: 8, lineHeight: 1.3 }}>{c.product}</div>
        </div>

        {/* slots + time */}
        <div style={{ display: "flex", gap: 12, marginTop: 12 }}>
          <div style={{ flex: 1, background: "#fff", borderRadius: 16, padding: 13, boxShadow: "0 4px 12px rgba(20,20,20,.06)" }}>
            <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 12.5, color: C.ink, lineHeight: 1.3 }}>Slots<br />Remaining</div>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 9, background: "#F2E9FD", borderRadius: 7, padding: "4px 9px", fontFamily: FONT_BODY, fontWeight: 700, fontSize: 11.5, color: C.purple }}>👥 {Math.max(0, c.slots - (isClaimed ? 1 : 0))} Slots left</span>
            <div style={{ height: 4, borderRadius: 6, background: "#FFE2D3", marginTop: 10, overflow: "hidden" }}>
              <div style={{ width: Math.round((c.slots / 50) * 100) + "%", height: "100%", background: "linear-gradient(90deg,#FF6D1D,#FFA26F)" }} />
            </div>
          </div>
          <div style={{ flex: 1, background: "#fff", borderRadius: 16, padding: 13, boxShadow: "0 4px 12px rgba(20,20,20,.06)" }}>
            <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 12.5, color: C.ink, lineHeight: 1.3 }}>Time<br />Remaining</div>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 9, background: "#FFE9E6", borderRadius: 7, padding: "4px 9px", fontFamily: FONT_BODY, fontWeight: 700, fontSize: 11.5, color: C.red }}>⏰ {c.days} days left</span>
          </div>
        </div>

        {/* how it works — 5 steps w/ icon tiles */}
        <div style={{ background: "#fff", borderRadius: 16, padding: "16px 15px", boxShadow: "0 4px 12px rgba(20,20,20,.06)", marginTop: 12 }}>
          <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 16, color: C.ink, marginBottom: 14 }}>How it works</div>
          {steps.map(([icon, title, sub], i) => (
            <div key={title} style={{ display: "flex", gap: 13, alignItems: "flex-start", position: "relative", paddingBottom: i < steps.length - 1 ? 18 : 0 }}>
              {i < steps.length - 1 && <span style={{ position: "absolute", left: 20, top: 42, bottom: 2, width: 0, borderLeft: "2px dashed #E4E5D6" }} />}
              <div style={{ width: 42, height: 42, borderRadius: 12, flex: "0 0 auto", display: "grid", placeItems: "center", fontSize: 20, background: ["#FDEBF0", "#EAF2FF", "#EAF2FF", "#F2E9FD", "#E9F7E4"][i], boxShadow: "0 3px 8px rgba(0,0,0,.06)" }}>{icon}</div>
              <div>
                <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 13.5, color: C.ink }}>{title}</div>
                <div style={{ fontFamily: FONT_BODY, fontWeight: 500, fontSize: 11.5, color: C.sub, marginTop: 2, lineHeight: 1.45 }}>{sub}</div>
              </div>
            </div>
          ))}
          <div style={{ marginTop: 14, background: "#FFFBE2", border: "1px solid #F3E9A8", borderRadius: 10, padding: "9px 11px", display: "flex", alignItems: "center", gap: 8 }}>
            <LogoMark size={20} />
            <span style={{ fontFamily: FONT_BODY, fontWeight: 500, fontSize: 11, color: C.sub }}>Your refund is credited to your <b style={{ color: C.ink }}>fayr Wallet</b></span>
          </div>
        </div>

        {/* FAQ */}
        <div style={{ marginTop: 16 }}>
          <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 15.5, color: C.ink, margin: "0 2px 8px" }}>Frequently Asked Questions</div>
          {FAQS.map(([q, a], i) => (
            <div key={i} onClick={() => setFaqOpen(faqOpen === i ? -1 : i)} style={{ background: "#fff", borderRadius: 12, padding: "12px 14px", marginBottom: 8, boxShadow: "0 3px 10px rgba(20,20,20,.05)", cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}>
                <span style={{ fontFamily: FONT_BODY, fontWeight: 600, fontSize: 12.5, color: C.ink, lineHeight: 1.45 }}>{q}</span>
                <span style={{ color: "#a3a496", transform: faqOpen === i ? "rotate(180deg)" : "none", transition: "transform .2s", flex: "0 0 auto" }}>⌄</span>
              </div>
              {faqOpen === i && <p style={{ fontFamily: FONT_BODY, fontWeight: 500, fontSize: 12, color: C.sub, margin: "9px 0 0", lineHeight: 1.55, animation: "fayr-rise .25s ease both" }}>{a}</p>}
            </div>
          ))}
        </div>

        {/* how to purchase — homepage search, exact product identity */}
        <div style={{ background: "#fff", borderRadius: 12, padding: "13px 14px", marginTop: 8, boxShadow: "0 3px 10px rgba(20,20,20,.05)" }}>
          <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 14, color: C.ink, marginBottom: 4 }}>How to purchase</div>
          <p style={{ fontFamily: FONT_BODY, fontWeight: 500, fontSize: 11, color: C.sub, margin: "0 0 11px", lineHeight: 1.5 }}>We open the {m.name} homepage — you search and buy the exact product yourself, from your own {m.name} account (the one on your fayr number). No affiliate or tracking links.</p>
          <div style={{ background: c.heroBg, borderRadius: 10, padding: "10px 12px", marginBottom: 11 }}>
            {[["Product", c.product], ["Brand", c.product.split(" ")[0]], ["Variant / size", c.variant], ["Marketplace", m.name]].map(([k, v], i, a) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "5px 0", borderBottom: i < a.length - 1 ? "1px solid rgba(0,0,0,.06)" : "none", fontFamily: FONT_BODY, fontSize: 11.5 }}>
                <span style={{ fontWeight: 600, color: C.sub }}>{k}</span><span style={{ fontWeight: 700, color: C.ink, textAlign: "right", maxWidth: "62%" }}>{v}</span>
              </div>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8, padding: "3px 0", fontFamily: FONT_BODY, fontWeight: 600, fontSize: 11, color: C.red, lineHeight: 1.45 }}>
            <span>⚠️</span><span>Buy only this exact product & variant. Any other product, size or seller won't be eligible for verification or refund.</span>
          </div>
        </div>

        {/* terms */}
        <div style={{ background: "#fff", borderRadius: 12, padding: "12px 14px", marginTop: 8, boxShadow: "0 3px 10px rgba(20,20,20,.05)" }}>
          <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 13.5, color: C.ink, marginBottom: 8 }}>Terms & conditions</div>
          {[`Your ${m.name} account must use the same mobile number or email you registered on fayr — they have to match`, `Buy the exact product & variant only: ${c.variant}`, "You'll be taken to the marketplace homepage — search for and buy the product yourself", "One entry per user", "Order & delivery are verified from your marketplace emails (or your uploaded screenshots) — not from the account link", "Reviews must be genuine — your rating never affects your refund", "Refund releases after the marketplace return window closes"].map((r) => (
            <div key={r} style={{ display: "flex", gap: 8, padding: "3px 0", fontFamily: FONT_BODY, fontWeight: 500, fontSize: 11.5, color: C.sub, lineHeight: 1.5 }}><span>•</span><span>{r}</span></div>
          ))}
        </div>

        {/* trust strip — calm reassurance at the decision point */}
        <div style={{ display: "flex", gap: 8, marginTop: 10, paddingBottom: 4 }}>
          {[["🔒", "Password never seen"], ["🧾", "Refund tracked upfront"], ["⭐", "Any rating pays same"]].map(([ic, t]) => (
            <div key={t} style={{ flex: 1, background: "#fff", borderRadius: 12, padding: "10px 8px", textAlign: "center", boxShadow: "0 3px 10px rgba(20,20,20,.04)" }}>
              <div style={{ fontSize: 17 }}>{ic}</div>
              <div style={{ fontFamily: FONT_BODY, fontWeight: 600, fontSize: 10, color: C.sub, marginTop: 4, lineHeight: 1.3 }}>{t}</div>
            </div>
          ))}
        </div>
      </div>

      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "12px 16px 18px", background: "linear-gradient(to top, #FBFBEF 72%, transparent)" }}>
        {!isClaimed ? (
          <Pill onClick={() => claim(c)} style={{ borderRadius: 16 }}>Claim this Product</Pill>
        ) : (
          <div>
            <div style={{ textAlign: "center", marginBottom: 8, fontFamily: FONT_DISPLAY, fontWeight: 800, fontStyle: "italic", fontSize: 14 }}>
              <span style={{ background: C.ink, color: "#fff", borderRadius: 5, padding: "2px 8px" }}>Slot Reserved</span>{" "}
              <span style={{ color: C.ink }}>for</span>{" "}
              <span style={{ color: C.red }}>{Math.floor(t / 60)}m : {String(t % 60).padStart(2, "0")}s</span>
            </div>
            <Pill onClick={() => buyNow(c)} color="linear-gradient(to bottom,#2E9E00,#1D7400)" style={{ borderRadius: 16, background: "linear-gradient(to bottom,#2E9E00,#1D7400)", boxShadow: "0 10px 22px rgba(46,158,0,.35)" }}>
              Buy Product on {m.name} →
            </Pill>
          </div>
        )}
      </div>
...
function ClaimedSheet({ go, c, buyNow }) {
  ...
  return (
    <Screen noPad bg="#FBFBEF">
      <div style={{ position: "absolute", inset: 0, background: "rgba(20,20,20,.5)", backdropFilter: "blur(2px)" }} onClick={() => go("detail")} />
      <div style={{ flex: 1 }} onClick={() => go("detail")} />
      <div style={{ position: "relative", margin: "0 14px 26px", background: "linear-gradient(180deg,#FDF6C9,#FFFDF2)", borderRadius: 22, padding: "22px 20px 24px", animation: "fayr-rise .35s ease both", textAlign: "center", boxShadow: "0 -10px 40px rgba(0,0,0,.25)" }}>
        <div style={{ width: 44, height: 4, background: "#E5DCA6", borderRadius: 4, margin: "0 auto 16px" }} />
        <div style={{ fontSize: 52, animation: "fayr-pop .5s ease both" }}>✔️</div>
...
function InsufficientSheet({ go, c, tickets, onClose }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 40, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1 }} />
      <div style={{ position: "relative", background: C.cream, borderTopLeftRadius: 26, borderTopRightRadius: 26, padding: "22px 22px 24px", animation: "fayr-rise .35s ease both" }}>
...
function NotifPrime({ go, c, setNotifAsked, dest }) {
  ...
  return (
    <Screen>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 30, textAlign: "center" }}>
        <div style={{ width: 84, height: 84, borderRadius: "50%", background: "#fff", display: "grid", placeItems: "center", fontSize: 40, boxShadow: "0 10px 24px rgba(0,0,0,.08)", animation: "fayr-pop .5s ease both" }}>🔔</div>
      ...
      <div className="fayr-scroll" style={{ flex: 1, overflowY: "auto", padding: "16px 16px 22px" }}>
        ...
        <div style={{ position: "relative" }}>
          {stages.map((stg, i) => {
            ...
            return (
              <div key={stg.key} style={{ display: "flex", gap: 14, position: "relative", animation: "fayr-rise .45s ease both", animationDelay: i * 45 + "ms" }}>
                {/* rail + node */}
  ...
  return (
    <Screen bg="#FBFBEF">
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "30px 28px", textAlign: "center", position: "relative" }}>
        ...
        {phase === "done" && (
          <div style={{ position: "relative", animation: "fayr-pop .5s ease both" }}>
      ...
      {phase !== "upload" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "30px 28px", textAlign: "center" }}>
          {phase === "verifying" ? (
            ...
            <div style={{ animation: "fayr-pop .5s ease both" }}>
              <div style={{ fontSize: 72 }}>✅</div>
              <h1 style={{ ...hTitle, fontSize: 23, marginTop: 12 }}>Delivery verified</h1>
  ...
  return (
    <Screen bg="rgba(20,20,20,.5)">
      ...
      <div style={{ position: "relative", background: "linear-gradient(180deg,#FBEE9E 0%,#FBFBEF 30%)", borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: "22px 26px 26px", animation: "fayr-rise .4s ease both", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 72, animation: "fayr-pop .5s ease both" }}>✅</div>
...
function ProgressRow({ c, e, delay, onAct, onOpen }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, padding: 12, marginBottom: 12, boxShadow: "0 6px 14px rgba(0,0,0,.07)", animation: "fayr-rise .5s ease both", animationDelay: delay + "ms" }}>
      <div onClick={onOpen} style={{ display: "flex", gap: 12, cursor: onOpen ? "pointer" : "default" }}>
        <div style={{ position: "relative", width: 88, height: 102, flex: "0 0 auto" }}>
          <ProductArt theme={c.theme} cid={c.id} radius={12} style={{ position: "absolute", inset: 0, background: c.heroBg }} />
          <div style={{ position: "absolute", left: "50%", bottom: -1, transform: "translateX(-50%)", background: "#fff", borderRadius: "8px 8px 0 0", padding: "2px 7px 1px" }}><BrandLogo mid={c.marketplace} size={20} /></div>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div className="fayr-scroll" style={{ flex: 1, overflowY: "auto", padding: "4px 20px 20px" }}>
        ...
        <CardBox style={{ marginTop: 12 }}>
          ...
          <p style={{ fontFamily: FONT_BODY, fontWeight: 500, fontSize: 12.5, color: C.sub, lineHeight: 1.6 }}>
            Start with 20 → joining locks a few → completing returns them <b style={{ color: C.ink2 }}>inside the reward celebration</b> — both currencies close their loop together.
          </p>
        </CardBox>
      </div>
    </Screen>
...
export default function FayrApp() {
  useGlobalStyle();
  ...
  const FLOW_GROUPS = [
    ...
    ["Claim flow", ["home", "howworks", "detail", "claimedsheet", "linkaccount", "redirect", "notifprime"]],
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#E9EBDD", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 12px", fontFamily: FONT_BODY }}>
      <div style={{ display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap", justifyContent: "center" }}>
        <div style={{ width: 360, height: 760, borderRadius: 44, background: "#000", padding: 10, boxShadow: "0 40px 80px rgba(0,0,0,.35)", flex: "0 0 auto" }}>
          <div style={{ position: "relative", width: "100%", height: "100%", borderRadius: 36, overflow: "hidden", background: C.cream }}>
            <DeviceStatusBar tone={["onboard"].includes(screen) ? "dark" : "dark"} />
            {screens[screen]}
          </div>
        </div>
        <div style={{ width: 216, flex: "0 0 auto" }}>
          ...
          <div className="fayr-scroll" style={{ maxHeight: 620, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
